...
class GameRepository {
  ...
  Future<Player?> getPlayer(int playerId) async {
    try {
      ...
      return Player(
        ...
        totalPowerEnemiesDefeated: playerData['total_power_enemies_defeated'] ?? 0,
        ...
        lastLevelUpAt: playerData['last_level_up_at'] != null
          ? DateTime.fromMillisecondsSinceEpoch(playerData['last_level_up_at'])
          : null,
        lastAchievementAt: playerData['last_achievement_at'] != null
          ? DateTime.fromMillisecondsSinceEpoch(playerData['last_achievement_at'])
          : null,
      );
    } catch (e, stackTrace) {
  ...
  Future<void> updatePlayer(int playerId, Player player) async {
    try {
      ...
      final playerData = {
        ...
        'last_played_at': player.lastPlayedAt?.millisecondsSinceEpoch ?? DateTime.now().millisecondsSinceEpoch,
    ...
    try {
      ...
      final primes = primeData.map((data) {
        return Prime(
          value: data['value'],
          count: data['count'],
          firstObtained: DateTime.fromMillisecondsSinceEpoch(data['first_obtained']),
  ...
  Future<void> completeBattle({
    ...
    try {
      ...
      final battleData = {
        ...
        'victory_claim_correct': victoryClaimCorrect != null ? (victoryClaimCorrect ? 1 : 0) : null,
  ...
  /// Get battle history
  Future<List<BattleResultModel>> loadBattleHistory({int? limit, int? playerId}) async {
    try {
      final currentPlayerId = playerId ?? await getCurrentOrCreatePlayer();
      Logger.info('Loading battle history for player: $currentPlayerId');
      
      final historyData = await _database.getPlayerBattleHistory(currentPlayerId, limit: limit);
  ...
  Future<List<Achievement>> getPlayerAchievements([int? playerId]) async {
    try {
      ...
      final achievementsData = await _database.getPlayerAchievementProgress(currentPlayerId);
      ...
      final achievements = achievementsData.map((data) {
        return Achievement(
          ...
          reward: _mapAchievementReward(data['reward_type'], data['reward_value'], data['reward_count']),
  ...
  Future<void> updateAchievementProgress({
    ...
    try {
      final currentPlayerId = playerId ?? await getCurrentOrCreatePlayer();
      Logger.info('Updating achievement progress: $achievementId for player: $currentPlayerId');
      
      final unlockedAt = isUnlocked == true ? DateTime.now().millisecondsSinceEpoch : null;
  ...
  /// Set game setting
  Future<void> setGameSetting(String key, String value, String type, [int? playerId]) async {
    try {
      final currentPlayerId = playerId ?? await getCurrentOrCreatePlayer();
  ...
  /// Import game data
  Future<void> importGameData(Map<String, dynamic> data, [int? playerId]) async {
    try {
      final currentPlayerId = playerId ?? await getCurrentOrCreatePlayer();
  ...
  AchievementCategory _mapAchievementCategory(String category) {
    switch (category) {
      case 'battle': return AchievementCategory.battle;
      case 'power_enemy': return AchievementCategory.powerEnemy;
      case 'speed': return AchievementCategory.speed;
      case 'efficiency': return AchievementCategory.efficiency;
      case 'collection': return AchievementCategory.collection;
      case 'progression': return AchievementCategory.progression;
  ...
  AchievementType _mapAchievementType(String type) {
    switch (type) {
      case 'milestone': return AchievementType.milestone;
      case 'cumulative': return AchievementType.cumulative;
      case 'streak': return AchievementType.streak;
      case 'percentage': return AchievementType.percentage;
      case 'time': return AchievementType.time;
      case 'secret': return AchievementType.secret;
      default: return AchievementType.milestone;
    }
  }
